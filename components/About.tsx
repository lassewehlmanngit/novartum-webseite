    ...
    <section className="bg-[#15171e] text-white py-24 md:py-32 relative overflow-hidden" id="about" aria-labelledby="about-heading">
      ...
      <div className="container mx-auto px-4 md:px-12 flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
        ...
        <div className="w-full lg:w-1/2 relative">
          <div className="relative z-10 grid grid-cols-2 gap-6">
             ...
             {features.map((feature, idx) => (
                <div key={idx} className={`bg-[#1f222b]/80 backdrop-blur-sm p-8 rounded-2xl border border-[#2a2e3b] shadow-2xl hover:-translate-y-1 transition-transform duration-300 ${idx === 2 ? 'col-span-2 max-w-sm mx-auto transform lg:-translate-y-6 hover:-translate-y-8' : idx === 0 ? 'transform lg:translate-y-12' : ''}`}>
                  <div className={`w-14 h-14 border rounded-xl flex items-center justify-center mb-5 ${idx === 0 ? 'bg-blue-900/50 border-blue-500/30 text-blue-400' : idx === 1 ? 'bg-orange-900/50 border-orange-500/30 text-orange-400' : 'bg-purple-900/50 border-purple-500/30 text-purple-400'}`}>
        ...
        <div className="w-full lg:w-1/2">
          <span className="text-orange-500 font-bold mb-3 block uppercase tracking-wide text-sm">{subtitle}</span>
          <h2 id="about-heading" className="text-3xl md:text-5xl font-bold mb-8 text-white leading-tight">
